...
func _draw() -> void:
	...
	var bottomleft: Vector2 = Vector2(hor_grad.front()[POINT].x, vert_grad.back()[POINT].y)
	var x_axis_start = bottomleft + (topleft - bottomleft) * max(0, float(vert_grad.back()[LABEL]) / (float(vert_grad.back()[LABEL]) - float(vert_grad.front()[LABEL])))
	var x_axis_end = Vector2(bottomright.x, x_axis_start.y)
	...
	if show_y_numbers == true:
		for grad in vert_grad:
			draw_string(default_font, grad[POINT] + Vector2(-35, -5), grad[LABEL])
